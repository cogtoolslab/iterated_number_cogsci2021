...
class FeatureExtractor():
    ...
    def extract_feature_matrix(self):
        ...
        def load_vgg19(layer_index=self.layer,use_cuda=False,cuda_device=self.cuda_device): # sebholt: use_cuda used to be True here
            if use_cuda == True:
            ...
            else: #sebholt added this condition
                vgg19 = models.vgg19(pretrained=True)
                vgg19 = VGG19Embeddings(vgg19,layer_index,spatial_avg=self.spatial_avg)
                vgg19.eval()  # freeze dropout
                print('NO CUDA.  CROP SKETCH set to {}'.format(self.crop_sketch))
        ...
        if use_cuda == True: #sebholt added this condition
            print('cuda device number: {}'.format(self.cuda_device))
        ...
        if generator:
            while True:
                ...
                for b in range(batch_size):
                    try:
                        sketch,sketchid = generator.__next__()
